...
function deploy_training_solution() {
  ...
  if [[ ${CONTINUOUS_TRAINING_ENABLED} == 'true' ]]; then
    ...
    export MODEL_RUNNER_PORT=`kubectl get svc -l trackingid=$TRACKING_ID -n $NAMESPACE -o json | jq -r '.items[0].spec.ports[0].port'`
...
NODEPORT=`kubectl get svc -l trackingid=$TRACKING_ID -n $NAMESPACE -o json | jq -r '.items[1].spec.ports[0].nodePort'`
